...
if (!$con) {
    die("Connection to Male database failed: " . mysqli_connect_error());
}

$con1 = mysqli_connect("localhost", "root", "", "hospitalms1");
if (!$con1) {
    die("Connection to Female database failed: " . mysqli_connect_error());
}

// Function to generate a new patient ID
function generatePID($current_db) {
    $query = "SELECT MAX(pid) AS max_pid FROM patreg;";
    $result = mysqli_query($current_db, $query);
    $row = mysqli_fetch_assoc($result);
    $max_pid = $row['max_pid'];
    $new_pid = (int)$max_pid + 1;
    return $new_pid;
}
...
if (isset($_POST['patsub1'])) {
    ...
    if ($gender == "Male") {
        ...
        // Handle other gender options or errors as needed
        header("Location: error.php");
        exit;
    }

    $pid = generatePID($current_db);
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $email = $_POST['email'];
    ...
    // Add the values to the database using an INSERT query
    $query = "INSERT INTO patreg(pid, fname, lname, gender, email, contact, password, cpassword) 
              VALUES ('$pid', '$fname', '$lname', '$gender', '$email', '$contact', '$password', '$cpassword');";
    $result = mysqli_query($current_db, $query);
...
if (isset($_POST['update_data'])) {
    $contact = $_POST['contact'];
    $status = $_POST['status'];
    $current_db = ($gender === "Male") ? $con : $con1;
